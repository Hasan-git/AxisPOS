    ...
    function AuthService($http, $q, session, __env, ngAuthSettings, $state, $timeout) {
        ...
        */
        vm.defaultPage = function () {
            ...
            switch (session.isInAnyRole(role)) {
                case session.isInAnyRole(['Vendor manager']):
                    $timeout(() => { $state.go('abstract.dashboard') }, 0)
                    break;
                case session.isInAnyRole(['Branch manager']):
                    $timeout(() => { $state.go('abstract.dashboard') }, 0)
                    break;
                case session.isInAnyRole(['Operators']):
                    $timeout(() => { $state.go('abstract.orders') }, 0)
                    break;
                case session.isInAnyRole(['Menu editor']):
                    $timeout(() => { $state.go('abstract.menu') }, 0)
                    break;
                default:
        ...
        */
        vm.logIn = function (loginData) {
            ...
            $http.post(__env.SecurityUrl + 'token', data,
        ...
        vm.refreshToken = function () {
            ...
            if (user != null) {
                ...
                if (user.useRefreshTokens) {
                    ...
                    var data = "grant_type=refresh_token&refresh_token=" + user.refreshToken + "&client_id=" + ngAuthSettings.clientId;

                    session.destroy();

                    $http.post(__env.SecurityUrl + 'token', data, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
                        .then(function (response) {
